...
var passwordList = []struct {
	line  string
	valid bool
}{
	{line: "1-3 a: abcde", valid: true},
	{line: "1-3 b: cdefg", valid: false},
	{line: "2-9 c: ccccccccc", valid: true},
}

func TestNewPolicy(t *testing.T) {
	testCases := []struct {
		policy   string
		expected day02.CharPolicy
	}{
		{policy: "1-3 a", expected: day02.CharPolicy{First: 1, Second: 3, Char: "a"}},
		{policy: "2-4 b", expected: day02.CharPolicy{First: 2, Second: 4, Char: "b"}},
	}

	for _, tc := range testCases {
		t.Run("Parses correctly: "+tc.policy, func(t *testing.T) {
			actual, err := day02.NewCharPolicy(tc.policy)
...
func TestIsPasswordWithinPolicy(t *testing.T) {
	for _, tc := range passwordList {
		t.Run(fmt.Sprintf("validate %s as %v", tc.line, tc.valid), func(t *testing.T) {
			parts := passwordLine(tc.line)

			p, err := day02.NewCharPolicy(parts[0])

			assert.NilError(t, err)
			assert.Assert(t, is.Equal(tc.valid, p.ValidCount(parts[1])))
		})
	}
}

func TestIsPasswordWithinPolicyPositional(t *testing.T) {
	passwordList := []struct {
		line  string
		valid bool
	}{
		{line: "1-3 a: abcde", valid: true},
		{line: "1-3 b: cdefg", valid: false},
		{line: "2-9 c: ccccccccc", valid: false},
	}
	for _, tc := range passwordList {
		t.Run(fmt.Sprintf("validate %s as %v", tc.line, tc.valid), func(t *testing.T) {
			parts := passwordLine(tc.line)
			...
			assert.NilError(t, err)
			assert.Assert(t, is.Equal(tc.valid, p.ValidPositions(parts[1])))
		})
	}
}

func passwordLine(s string) []string {
...
func TestBigOneCount(t *testing.T) {
	lines := strings.Split(`1-2 x: xpxc
1-5 b: bwlbbbbcq
3-5 v: qvjjdhvl
...
`, "\n")
	...
	for _, l := range lines {
		...
		s := passwordLine(l)
		p, err := day02.NewCharPolicy(s[0])

		assert.NilError(t, err, "couldn't create policy: %v", s)

		if p.ValidCount(s[1]) {
			policyOne++
		}
		if p.ValidPositions(s[1]) {
			policyTwo++
		}
	}

	t.Logf("ValidCount passwords: %d", policyOne)
	t.Logf("ValidPositions passwords: %d", policyTwo)
}
